...
static const char *FRAGMENT_SHADER[] = {
    ...
    "",
    "vec4 oklch_to_oklab(vec4 lch) {",
    "    float c = lch.y;",
    "    float h = lch.z * 2.0 * 3.14159;",
    "    return vec4(lch.x, c * cos(h), c * sin(h), lch.w);",
    "}",
    "vec4 oklab_to_rgb(vec4 c) {",
    ...
    "            cspace == 1? gamma(oklab_to_rgb(oklch_to_oklab(color))):",
    "            cspace == 2? gamma(oklab_to_rgb(color)):",
    "            cspace == 3? gamma(xyz_to_rgb(lab_to_xyz(lchab_to_lab(color)))):",
    "            cspace == 4? gamma(xyz_to_rgb(lab_to_xyz(color))):",
    "            cspace == 5? gamma(xyz_to_rgb(color)):",
    "            gamma(color);",
    "}",
...
_clear(Pg *g)
{
    ...
    if (paint->nstops == 1) {
        ...
        PgColor c = pg_convert_color_to_rgb(paint->cspace,
                                            paint->colors[0],
                                            g->s.gamma);
