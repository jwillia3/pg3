...
#define OTF(FONT) ((OpenTypeFont*) (FONT))->share
...
static const PgFontFunc methods;
...
ttoutline(Pg *g, PgFont *font, PgTM ctm, unsigned glyph)
{
    ...
    if ((signed) ncontours > 0) {
        ...
        for (unsigned n = 0; n < npoints; fi++) {
            ...
            while (rep-- > 0) {
                ...
                oldp = p;
                p = pg_mat_apply(ctm, pgpt(x, y));

                if (contourstart) {                     // Closing subpath
                    next = PW(ends + ci++ * 2) + 1;
                    if (curving)
                        pg_canvas_curve3(g, oldp.x, oldp.y, home.x, home.y);
                    pg_canvas_close_path(g);
                    pg_canvas_move(g, p.x, p.y);
                    home = p;
                    curving = false;
                }
                else if (anchor && curving) {           // Curve-to-line
                    pg_canvas_curve3(g, oldp.x, oldp.y, p.x, p.y);
                    curving = false;
                }
                else if (anchor)                        // Line-to-line
                    pg_canvas_line(g, p.x, p.y);
                else if (curving) {                     // Line-to-curve
                    PgPt m = midpoint(oldp, p);
                    pg_canvas_curve3(g, oldp.x, oldp.y, m.x, m.y);
                }
                else                                    // Curve-to-curve
                    curving = true;
            }
        ...
        if (npoints != 0) {
            if (curving)
                pg_canvas_curve3(g, p.x, p.y, home.x, home.y);
            pg_canvas_close_path(g);
        }
    ...
    // Composite Glyph.
        ...
        while (true) {
            ...
            ttoutline(g, font, pg_mat_multiply(tm, ctm), newglyph);
...
rmove(Pg *g, PgTM ctm, PgPt a, PgPt b) {
    ...
    PgPt    tb = pg_mat_apply(ctm, b);

    pg_canvas_close_path(g);
    pg_canvas_move(g, tb.x, tb.y);
...
rcurve(Pg *g, PgTM ctm,  PgPt a, PgPt b, PgPt c, PgPt d) {
    ...
    PgPt    tb = pg_mat_apply(ctm, b);
    PgPt    tc = pg_mat_apply(ctm, c);
    PgPt    td = pg_mat_apply(ctm, d);

    pg_canvas_curve4(g, tb.x, tb.y, tc.x, tc.y, td.x, td.y);
...
PgFont*
pg_font_from_data_otf(const uint8_t *data, size_t filesize, unsigned index)
{
    ...
    cursect = os2;
    {
        ...
        ascender = (int16_t) PW(68);
        descender = (int16_t) PW(70);
    }
    ...
    return pgnew(OpenTypeFont,
                 .font = _pg_font_init(&methods,
                                      data,
                                      filesize,
                                      index,
                                      nglyphs,
...
_clone(PgFont *font)
{
    OpenTypeFont    *clone = malloc(sizeof *clone);
    clone->font = *font;
    clone->share = OTF(font);
    clone->share->refs++;
    return &clone->font;
}
...
serif_style(PgFont *font)
{
    ...
    const char *middle = pg_stristr(family, " sans ");
    const char *beginning = pg_stristr(family, "sans ");
    const char *end = pg_stristr(family, " sans");
...
{
    PgPt    size = pg_font_get_scale(font);
    float   sx = size.x;
    float   sy = size.y;
    ...
    case PG_FONT_UNITS:         return font->units;
...
_int(PgFont *font, PgFontProp id)
{
    ...
    case PG_FONT_UNITS:         return (int) font->units;
    case PG_FONT_EM:            return sy * (int) font->units;
    case PG_FONT_AVG_WIDTH:     return sx * (int16_t) TW(os2, 2);
    case PG_FONT_ASCENDER:      return sy * (int16_t) TW(os2, 68);
    case PG_FONT_DESCENDER:     return sy * (int16_t) TW(os2, 70);
    case PG_FONT_LINEGAP:       return sy * (int16_t) TW(os2, 72);
...
_string(PgFont *font, PgFontProp id)
{
    ...
    case PG_FONT_SUP_SX:
    case PG_FONT_SUP_SY:
        sprintf((char*) buf, "%g", pg_font_prop_float(font, id));
        return (char*) buf;
    }
...
{
    if (!pg_canvas_get_underline(g))
        return;

    float   w = pg_font_measure_glyph(font, glyph);
    float   a = pg_font_prop_float(font, PG_FONT_UNDERLINE);
    float   t = pg_font_prop_float(font, PG_FONT_UNDERLINE_SIZE);

    // CFF and TTF tend to have opposite windings.
    if (OTF(font)->cffver == 0) {
        pg_canvas_move(g, x, y - a + t);
        pg_canvas_rline(g, w, 0.0f);
        pg_canvas_rline(g, 0.0f, -t);
        pg_canvas_rline(g, -w, 0.0f);
        pg_canvas_close_path(g);
    }
    else {
        pg_canvas_move(g, x, y - a);
        pg_canvas_rline(g, w, 0.0f);
        pg_canvas_rline(g, 0.0f, t);
        pg_canvas_rline(g, -w, 0.0f);
        pg_canvas_close_path(g);
...
{
    y += font->ascender * font->sy;

    PgPt    scale = pg_font_get_scale(font);
    PgTM    ctm = { scale.x, 0.0f, 0.0f, -scale.y, x, y };
...
_measure_glyph(PgFont *font, unsigned glyph)
{
    ...
    return raw * pg_font_get_scale(font).x;
}
